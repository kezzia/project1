...
(define (reverse-general L)
  (cond
    ((null? L)L) ;if the list is empty return the empty list
    ((list? L) ;if the list is not empty, take the first element and append it to the list recursively
      (append (reverse-general (cdr L))(list (reverse-general (car L)))
    ))
    (else L) ;returns the call stack
  )
)
